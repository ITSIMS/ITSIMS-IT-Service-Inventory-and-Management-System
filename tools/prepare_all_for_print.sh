...
# Скрипт для подготовки всех документов проекта к печати
# Конвертирует все Markdown файлы в PDF с сохранением структуры проекта
...
# Функция для подготовки markdown файла к печати
prepare_markdown_for_print() {
    local input_file="$1"
    local output_file="$2"
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "❌ Ошибка при подготовке к печати $input_file"
        return 1
    fi
}
...
process_directory() {
    ...
    for item in "$source_dir"/*; do
        ...
        # Пропускаем папку print, чтобы избежать рекурсии
        if [[ "$item_name" == "print" ]]; then
            echo "⏭️  Пропускаем папку print (избегаем рекурсии)"
            continue
        fi
        ...
        if [ -d "$item" ]; then
            ...
            # Если это markdown файл, подготавливаем к печати
            if [[ "$item" == *.md ]]; then
                local pdf_name="${item_name%.md}.pdf"
                local pdf_path="$dest_dir/$pdf_name"
                prepare_markdown_for_print "$item" "$pdf_path"
...
# Основная функция
main() {
    echo "🚀 Начинаем подготовку всех документов к печати..."
    echo "📁 Исходная директория: $PROJECT_ROOT"
    echo "📁 Целевая директория: $OUTPUT_DIR"
    echo ""
    
    # Создаем корневую директорию для вывода (print)
    ...
    echo ""
    echo "🎉 Подготовка к печати завершена!"
    echo "📁 Все документы готовы к печати в: $OUTPUT_DIR"
}
